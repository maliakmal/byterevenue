<div>
    ...
    <input type="hidden"  name="{{ $selectorName }}" class="text-gray-700 " wire:model="inputType" value="file"> Upload CSV (name, phone, email)
    <input type="file" onchange="{{$onChange}}" name="{{ $fileInputName }}"  wire:model="file" class="  appearance-none   rounded w-full py-3 p-3 text-gray-700 focus:outline-none focus:shadow-outline" />
    </div>
</div>
