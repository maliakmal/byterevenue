...
<header class="bg-gray-50 py-8">
  ...
  <path fill-rule="evenodd" d="M7.21 14.77a.75.75 0 01.02-1.06L11.168 10 7.23 6.29a.75.75 0 111.04-1.08l4.5 4.25a.75.75 0 010 1.08l-4.5 4.25a.75.75 0 01-1.06-.02z" clip-rule="evenodd"></path>
                  ...
                  <a href="#" class="ml-4 text-sm font-medium text-gray-500 hover:text-gray-700">Edit Campaigns</a>
                </div>
              </li>
            </ol>
          </nav>
          <h1 class="mt-2 text-2xl font-bold leading-7 text-gray-900 sm:truncate sm:text-3xl sm:tracking-tight">Edit Campaign</h1>
      ...
      <div>
        ...
        <form action="{{ route('campaigns.update', $campaign->id) }}" method="post">
            ...
            <div class="mb-4">
              ...
              <textarea class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" id="description" name="description" >{{ $campaign->description }}</textarea>
            </div>
            <div class="mb-4">
              ...
              <select id="recipients_list_id" name="recipients_list_id" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" >
                ...
                @foreach ($recipient_lists as $recipient_list)
                    <option {{ $campaign->recipients_list_id == $recipient_list->id ? 'selected':'' }} value="{{ $recipient_list->id }}">{{ $recipient_list->name }}</option>
                @endforeach
              </select>
            </div>
            <div class="mb-4">
            ...
            <a href="/campaigns/{{ $campaign->id }}" class="inline-flex items-center rounded-md border border-gray-300 bg-white py-2 px-4  font-medium text-gray-700 shadow-sm hover:bg-gray-50 ">Back</a>
          </form>
        </div>
      </div>
    </div>
